...
func addUsersToBucket(userBucket *userBucketStorageObject, userId string, buckbucketSize int, usePivot bool, ctx context.Context, logger runtime.Logger, db *sql.DB) error {
	var rows *sql.Rows
	var err error
	if usePivot {
		pivotID := uuid.Must(uuid.NewV4(), nil).String()
		row, err = db.QueryContext(ctx, `SELECT id FROM users WHERE id > $1 LIMIT $2`, pivotID, bucketSize)
	} else {
	...
	if err := rows.Err(); err != nil {
		logger.Error("rows.Error error: %v", err)
		return ErrInternalError
	}
	return nil
}
